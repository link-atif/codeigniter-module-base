...
class Ion_auth_model extends CI_Model
{
	...
	{
		$this->db->where('id',$id);
		$qry = $this->db->get("users")->row();
		return $qry;
	}
	public function updateUserActiveStatus($id,$params)
	{
		$this->db->where('id',$id);
		$qry = $this->db->update("users",$params);
		return $qry;
	}
	public function hash_password($password, $identity = NULL, $use_sha1_override=FALSE)
	{
		if (empty($password))
		{
			return FALSE;
	...
	public function reset_password($identity, $new) {
		...
		$new = $this->hash_password($new, $identity);
	 ...
	 * @author Mathew
	 **/
	public function change_password($identity, $username, $old, $new)
	{
		$this->trigger_events('pre_change_password');
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $type, $email, $additional_data = array(), $groups = array())
	{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				$this->increase_login_attempts($user->id);
	...
	public function increase_login_attempts($identity) {
		...
		//if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			return $this->db->insert($this->tables['login_attempts'], array('ip_address' => $ip_address, 'login' => $identity, 'time' => date('Y-m-d H:i:s')));
		//}
		return FALSE;
	}
