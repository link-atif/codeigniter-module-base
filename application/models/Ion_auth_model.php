...
class Ion_auth_model extends CI_Model
{
	...
	public function forgotten_password($identity)
	{
		...
		$update = [
			...
			'forgotten_password_code' => str_replace("/", "", $token->validator_hashed),
	...
	public function register($identity, $password, $first_name,$last_name, $additional_data = [], $groups = [],$api_check)
	{
		...
		if($additional_data['phone']!=""){
			if ($this->phone_check($additional_data['phone'])){
		...
		$user_data['added_by'] = 'self';
		$qry = $this->db->insert($this->tables['users'], $user_data);
		...
		// echo $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_activate_web', 'ion_auth'));
		// 	die();
	
		if(is_array($api_check) || $api_check == 'false'){
			
			//sending email for web only

			$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_activate_web', 'ion_auth'), $data, true);

			$email_data['to_ids'] = $user->email;
		
			$email_data['subject'] = $this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_activation_subject');
			$email_data['content'] = $message;
			$res = _send_email($email_data);
		}
		if($api_check == true){
			// Do nothing
		}
	 ...
	 * @author    Mathew
	 */
	public function login($identity, $password, $remember=FALSE,$from_api='')
	{
		...
		$query = $this->db->select($this->identity_column . ', username, phone, id, password, active, last_login')
		->where($this->identity_column, $identity)
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->password, $identity))
			{
				if ($user->active == 0)
				{
					...
					// 	'message1',
					// 	"Your Account is inactive.Please click to activation link to your email before login<br><br><p onclick=resendactivationemail(this); user_email =".$identity." style='background: #4a1798;border-radius: 10px;text-align: center;cursor: pointer;' >Click here to send email again</p>"
					// );// 	
	...
	public function check_user_is_active($identity, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
	...
	public function get_users_data_by_group($id = NULL)
	{
		$this->db->select('gr.user_id, u.*');
		$this->db->join('users u','gr.user_id= u.id');
		$this->db->from('users_groups gr');
		$this->db->where('gr.group_id', $id);
		return $query = $this->db->get()->result();
	}
	public function get_users_by_group_id($id = NULL, $offset = NULL)
	{
		$this->db->select('gr.user_id, u.*');
		$this->db->join('users u','gr.user_id= u.id');
		$this->db->from('users_groups gr');
	...
	public function get_find_deals_courier()
	{
		date_default_timezone_set("Asia/Karachi");
		$selectedTime = date("Y-m-d H:i:s");
		$endTime = strtotime("-350 minutes", strtotime($selectedTime));
		$endTime = date('Y-m-d H:i:s', $endTime);
		$query = $this->db->query('SELECT * FROM `find_deals_courier` WHERE added_on >= "'.$endTime.'" AND deal_status = 1');
		$data = $query->result_array();
		// return $this->db->last_query();
		return $data;
	}
	public function get_deal_courier($courier_id)
	{
		$query = $this->db->query('SELECT * FROM `find_deals_courier` WHERE id >= "'.$courier_id.'"');
		$data = $query->row_array();
		// return $this->db->last_query();
		return $data;
	}
	public function update_find_deals_courier($id,$last_courier)
	{
		$this->db->where('id' , $id);
		$this->db->update('find_deals_courier', array('last_send_email' => $last_courier));
		// return $this->db->last_query();
		return $query;
	}
	public function get_deal_by_id($id = NULL)
	{
		$this->db->select('*');
		$this->db->from('find_deals fd');
	...
	public function user_online_status($id = NULL)
	{
		// if no id was passed use the current users id
		$id = isset($id) ? $id : $this->session->userdata('user_id');

		$this->db->update($this->tables['users'], array('is_online'=>1), ['id' => $id]);
		$params['user_id'] = $id;
        $params['log_in_datetime'] = date("Y-m-d H:i:s");
        $params['added_on'] = date("Y-m-d H:i:s");
        $params['status'] = '1';
        $this->db->insert('user_logs',$params);

		return $this;
	}
	public function user_offline_status($id = NULL)
	{
		// if no id was passed use the current users id
		$id = isset($id) ? $id : $this->session->userdata('user_id');

		$this->db->update($this->tables['users'], array('is_online'=>0), ['id' => $id]);
...
// session_start();
	...
	protected function _get_hash_parameters($identity = NULL)
	{
		...
		switch ($this->hash_method)
		{
			...
			case 'mcrypt':
			$params = [
				'cost' => $is_admin ? $this->config->item('mcrypt_admin_cost', 'ion_auth')
				: $this->config->item('mcrypt_default_cost', 'ion_auth')
			];
			break;
